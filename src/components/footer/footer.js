...
function footerTemplate(storeColor) {
  let footerArray = [
    ...
    '<section class="ui very padded basic inverted vertical site segment">',
    '<div class="container">',
    '<nav class="row">'
  ];
  const company = [
  ...
  const retail = [
    ...
    { name: "Multiples Discount", link: "https://organicstart.com/multiples" },
    {
  ...
  const article = [{ name: "RECENT ARTICLES" }, { id: "recent-post" }];

  footerArray.push(appendItem(company));
  footerArray.push(appendItem(retail));
  footerArray.push(appendItem(resources));
  footerArray.push(appendItem(article));
  footerArray.push("</nav><div></section></footer>");
  return footerArray.join("");
}

function appendItem(items) {
  let itemsArray = items.map((data, index) => {
    if (index === 0) {
      return `<ul class="col-md-3 col-lg-3 col-12 nav flex-column text-center text-md-left ui vertical inverted text menu footer-menu mt-0 mb-0 pt-0 pb-0"
